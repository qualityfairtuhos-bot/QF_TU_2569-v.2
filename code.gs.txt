...
const DB_SCHEMA = Object.freeze({
  ...
  Registrations: ['RegID','ConferenceID','SourceType','SourceBatchID','SourceRowNo','SourceTimestamp','SourceRespondentEmail','ConsentAccepted','ConsentVersion','ParticipantType','Region4Status','Prefix','FirstName','LastName','FullName','Position','OrganizationGroup','OrganizationUnit','Institution','Profession','LicenseNo','CID','Passport','Phone','LineID','Email','ReceiptRequirement','ReceiptName','ReceiptTaxID','ReceiptAddress','ReceiptPostalCode','ReceiptPhone','NeedInvoice','InvoiceStatus','InvoiceNote','FoodType','FoodAllergyDetail','AttendanceDay1','AttendanceDay2','AttendanceDay3','WantsSubmitWork','DataCompletenessStatus','RegistrationStatus','PaymentStatus','MealPassStatus','EditAccessCodeHash','Note','CreatedAt','UpdatedAt','LastModifiedBy'],
  Consents: ['ConsentID','ConferenceID','RegID','ConsentVersion','ConsentAccepted','AcceptedAt','ClientInfo'],
  Payments: ['PaymentID','ConferenceID','RegID','Amount','Currency','PaymentChannelID','SlipFileId','SlipFileUrl','SlipFileName','Status','SubmittedAt','VerifiedBy','VerifiedAt','FinanceNote','ReceiptNo','ReceiptDate','ReceiptStatus','ReceiptFileId','ReceiptFileUrl','InvoiceFileId','InvoiceFileUrl','InvoiceFileName','ReceiptFileName','OtherFilesJson','CreatedAt','UpdatedAt'],
  FinanceDocuments: ['DocumentID','ConferenceID','RegID','PaymentID','DocType','FileName','FileId','FileUrl','MimeType','FileSize','UploadedBy','UploadedAt','Note','Active'],
...
const PLAIN_TEXT_FIELDS = Object.freeze(['CID','Passport','Phone','LicenseNo','AccountNumber','PromptPay','ReceiptNo','RegID','WorkCode','Postcode','ReceiptTaxID','ReceiptPostalCode','ReceiptPhone','DocumentID']);
...
function cachePutLarge_(key, data, ttlSeconds) {
  try {
    const cache = CacheService.getScriptCache();
    const str = typeof data === 'string' ? data : JSON.stringify(data);
    const ttl = Math.min(Math.max(1, ttlSeconds || 300), 21600);
    ...
    for (let i = 0; i < totalChunks; i++) {
      entries[key + '_' + i] = str.substr(i * CHUNK_SIZE, CHUNK_SIZE);
    }
    entries[key + '_meta'] = JSON.stringify({ chunks: totalChunks });
...
function cacheGetLarge_(key) {
  try {
    ...
    if (single) {
      try { return JSON.parse(single); } catch(e) { return single; }
    }
    ...
    let full = '';
    for (let i = 0; i < meta.chunks; i++) {
      const part = chunkMap[key + '_' + i];
      if (part === undefined || part === null) return null;
      full += part;
    }
    try { return JSON.parse(full); } catch(e) { return full; }
  } catch(e) {
    return null;
  }
}
...
function getRecords_(name){
  ...
  if (isCachable) {
    try {
      ...
      if (cached && Array.isArray(cached)) {
        globalThis.__TUH_RECORDS[name] = cached;
        return cached;
      }
    } catch(e) {}
  }
  ...
  if (isCachable) {
    try {
      cachePutLarge_('TBL_' + name, records, isMaster ? 600 : 180);
    } catch(e) {}
  }
  return records;
}
function clearTableCache_(name){
  try {
    cacheRemoveLarge_('TBL_' + name);
    if (globalThis.__TUH_RECORDS) delete globalThis.__TUH_RECORDS[name];
  } catch(e){}
}
function clearRequestCache_(){ globalThis.__TUH_RECORDS={}; }
...
function loginUser(username,password,conferenceId,clientInfo,options){
  return runSafely_('loginUser',function(){
    ...
    if (opt.portal === 'reviewer' || resolvedRole === 'REVIEWER') {
      try {
        result.reviewerBoot = reviewerBootstrapInternal_(user, resolvedRole, cid);
      } catch(e) {}
    }
    return result;
  });
...
function uploadExcelForImport(token, conferenceId, file) {
  return runSafely_('uploadExcelForImport', function() {
    ...
    try {
      ...
      const fileName = 'IMPORT_' + Date.now() + '_' + (file.name || 'registration.xlsx');

      if (typeof Drive !== 'undefined' && Drive.Files && typeof Drive.Files.insert === 'function') {
        // Drive API v2 (Default in Google Apps Script UI)
        const resource = {
          title: fileName,
          mimeType: 'application/vnd.google-apps.spreadsheet'
        };
        if (folderMap['01_Import_Temp']) {
          resource.parents = [{ id: folderMap['01_Import_Temp'] }];
        }
        try {
          converted = Drive.Files.insert(resource, blob, { convert: true });
        } catch (errInsert) {
    ...
    const fileId = converted && (converted.id || (typeof converted.getId === 'function' ? converted.getId() : null));
    if (!fileId) throw new Error('ไม่สามารถแปลงไฟล์เป็น Google Sheet ได้');
    ...
    if (!source) source = SpreadsheetApp.openById(fileId);
...
function publicRegistration_(r){const out={};['RegID','ParticipantType','Prefix','FirstName','LastName','FullName','Position','OrganizationGroup','OrganizationUnit','Institution','Profession','LicenseNo','CID','Phone','LineID','Email','FoodType','FoodAllergyDetail','AttendanceDay1','AttendanceDay2','AttendanceDay3','WantsSubmitWork','ReceiptRequirement','ReceiptName','ReceiptTaxID','ReceiptAddress','ReceiptPostalCode','ReceiptPhone','NeedInvoice','InvoiceStatus','InvoiceNote','DataCompletenessStatus','RegistrationStatus','PaymentStatus','MealPassStatus','CreatedAt','UpdatedAt'].forEach(function(k){out[k]=r[k];});return serialize_(out);}
...
function getPublicFinanceDocuments(conferenceId, regId, emailOrPhone) {
  return runSafely_('getPublicFinanceDocuments', function() {
    ...
    const docs = findMany_('FinanceDocuments', {ConferenceID: cid, RegID: r.RegID}).filter(function(d) {
      return bool_(d.Active);
    });
    docs.sort(function(a, b) {
      return new Date(b.UploadedAt).getTime() - new Date(a.UploadedAt).getTime();
    });
    const publicDocs = docs.map(function(d) {
      return {
...
function adminListReviewers(token,conferenceId){
  return runSafely_('adminListReviewers',function(){
    ...
    const serialized = serialize_(result);
    try {
      cachePutLarge_(cacheKey, serialized, 180);
    } catch(e) {}
    return serialized;
  });
}
...
function reviewerBootstrapInternal_(user, userRole, cid) {
  const earlyCacheKey = 'REV_BOOT_' + cid + '_' + (user.UserID || user.Email);
  try {
    const cached = cacheGetLarge_(earlyCacheKey);
    if (cached) return cached;
  } catch(e) {}

  // Find reviewer ID with resilient multi-step resolution
  const role = findOne_('UserConferenceRoles', {ConferenceID: cid, UserID: user.UserID, Role: 'REVIEWER'}) ||
               findOne_('UserConferenceRoles', {ConferenceID: cid, UserID: user.UserID});
  ...
  if (!reviewer && user.UserID) {
    reviewer = findOne_('Reviewers', {UserID: user.UserID});
  ...
  if (!reviewer) {
    ...
    reviewer = {
      ...
      FullName: user.FullName || [user.Prefix, user.FirstName, user.LastName].filter(Boolean).join(' ') || user.Username,
  ...
  const reviewerCacheKey = 'REV_BOOT_' + cid + '_' + reviewerId;
  try {
    const cached = cacheGetLarge_(reviewerCacheKey);
    if (cached) return cached;
  } catch(e) {}
  ...
  try {
    cachePutLarge_(earlyCacheKey, bootData, 180);
    if (reviewerId) {
      cachePutLarge_(reviewerCacheKey, bootData, 180);
    }
  } catch(e) {}
  return bootData;
}
...
function reviewerBootstrap(token,conferenceId){
  return runSafely_('reviewerBootstrap',function(){
    ...
    const cid = conferenceId || APP.DEFAULT_CONFERENCE_ID;
    return reviewerBootstrapInternal_(ctx.user, ctx.role, cid);
  });
}
...
function adminSetupCategoryScoringCriteria(token,conferenceId,roundId){
  return runSafely_('adminSetupCategoryScoringCriteria',function(){
    ...
    var serviceCriteria = [
      ...
      { ItemNo: 3, NameTH: 'กระบวนการพัฒนาและการนำไปใช้จริง', MaxScore: 25, Desc: 'ขั้นตอนการออกแบบ การมีส่วนร่วม การนำกระบวนการหรือนวัตกรรมบริการไปปฏิบัติ' },
    ...
    var researchCriteria = [
      { ItemNo: 1, NameTH: 'ความสำคัญและความเป็นมาของปัญหาการวิจัย / นวัตกรรม', MaxScore: 15, Desc: 'ความสำคัญ ปัญหา และช่องว่างทางวิชาการหรือนวัตกรรม' },
      { ItemNo: 2, NameTH: 'คำถามวิจัย / วัตถุประสงค์ / สมมติฐาน', MaxScore: 10, Desc: 'มีความชัดเจน สอดคล้องกับปัญหาและกรอบแนวคิด' },
      { ItemNo: 3, NameTH: 'ระเบียบวิธีวิจัย / การออกแบบสิ่งประดิษฐ์และนวัตกรรม', MaxScore: 20, Desc: 'แบบแผนการวิจัย หรือการออกแบบสิ่งประดิษฐ์ถูกต้องตามหลักวิชาการ' },
      { ItemNo: 4, NameTH: 'คุณภาพเครื่องมือและการเก็บรวบรวมข้อมูล / จริยธรรมการวิจัย', MaxScore: 15, Desc: 'ความเที่ยงตรง ความน่าเชื่อถือ และการพิทักษ์สิทธิ์กลุ่มตัวอย่าง' },
      { ItemNo: 5, NameTH: 'การวิเคราะห์ข้อมูลและผลการวิจัย / ประสิทธิผล', MaxScore: 20, Desc: 'สถิติหรือวิธีการวิเคราะห์เหมาะสม ผลการวิจัยตอบโจทย์' },
      { ItemNo: 6, NameTH: 'การอภิปรายผล / ข้อเสนอแนะและการนำไปใช้ประโยชน์', MaxScore: 10, Desc: 'อภิปรายสอดคล้องกับผลวิจัยและทฤษฎี ข้อเสนอแนะนำไปใช้ได้จริง' },
      { ItemNo: 7, NameTH: 'ความคิดริเริ่มสร้างสรรค์ / คุณค่าต่องานสุขภาพ', MaxScore: 10, Desc: 'ความแปลกใหม่ ประโยชน์ต่อการพัฒนาสุขภาพและระบบบริการ' }
    ];

    clearTableCache_('ScoringCriteria');
    var currentCrit = findMany_('ScoringCriteria',{ConferenceID:cid});

    function syncGroupCriteria(items, catId, groupName) {
      items.forEach(function(it){
        var found = currentCrit.find(function(c){
          return String(c.ConferenceID) === String(cid) &&
                 String(c.CategoryID).toUpperCase() === catId.toUpperCase() &&
                 num_(c.ItemNo) === it.ItemNo;
        });
        var patch = {
          ConferenceID: cid,
          ReviewRoundID: targetRoundId,
          CategoryID: catId,
          PresentationTypeID: 'ALL',
          ItemNo: it.ItemNo,
          CriteriaNameTH: it.NameTH,
          CriteriaNameEN: groupName + ' Criterion ' + it.ItemNo,
          DescriptionTH: it.Desc,
        ...
        if (found) {
          updateRecord_('ScoringCriteria', found.__row, patch);
        } else {
          appendRecord_('ScoringCriteria', Object.assign({ CriteriaID: nextId_('CRIT') }, patch));
        }
      });
    }
...
function adminGetReviewConfig(token,conferenceId){
  return runSafely_('adminGetReviewConfig',function(){
    requireSession_(token,['SUPERADMIN','CONFERENCE_ADMIN','ACADEMIC_STAFF'],conferenceId);
    var cid = conferenceId || APP.DEFAULT_CONFERENCE_ID;
    var cacheKey = 'ADM_RCFG_' + cid;
    try {
      var cached = CacheService.getScriptCache().get(cacheKey);
      if (cached) return JSON.parse(cached);
    } catch(e) {}
    var res = {
      rounds: serialize_(findMany_('ReviewRounds',{ConferenceID:cid})),
      criteria: serialize_(findMany_('ScoringCriteria',{ConferenceID:cid})),
      categories: serialize_(findMany_('WorkCategories',{ConferenceID:cid})),
      presentations: serialize_(findMany_('PresentationTypes',{ConferenceID:cid}))
    };
    try {
      CacheService.getScriptCache().put(cacheKey, JSON.stringify(res), 300);
    } catch(e) {}
    return res;
  });
}
...
function adminSaveRegistration(token,conferenceId,regId,payload){
  return runSafely_('adminSaveRegistration',function(){
    const ctx=requireSession_(token,['SUPERADMIN','CONFERENCE_ADMIN','REGISTRATION_STAFF','FINANCE_STAFF'],conferenceId),r=findOne_('Registrations',{ConferenceID:conferenceId,RegID:regId});if(!r)throw new Error('ไม่พบผู้ลงทะเบียน');payload=payload||{};
    ...
    const patch={
      ParticipantType:participantType,
      Region4Status:clean_(payload.Region4Status||r.Region4Status),
      Prefix:prefix,
      FirstName:first,
      LastName:last,
      ...
      Institution:clean_(payload.Institution||org),
      Profession:clean_(payload.Profession),
      LicenseNo:clean_(payload.LicenseNo),
      CID:cid,
      Phone:phone,
      ...
      InvoiceStatus:payload.InvoiceStatus !== undefined ? clean_(payload.InvoiceStatus) : r.InvoiceStatus,
      FoodType:clean_(payload.FoodType),
      FoodAllergyDetail:clean_(payload.FoodAllergyDetail),
      AttendanceDay1:bool_(payload.AttendanceDay1),
      AttendanceDay2:bool_(payload.AttendanceDay2),
...
function adminListUsers(token,conferenceId){
  return runSafely_('adminListUsers',function(){
    ...
    const result = serialize_(users.map(function(u){
    ...
    try {
      CacheService.getScriptCache().put(cacheKey, JSON.stringify(result), 120);
    } catch(e) {}
    return result;
  });
}
...
function exportWorksToExcel(token,conferenceId){
  return runSafely_('exportWorksToExcel',function(){
    ...
    const asc=assigns.reduce(function(a,c){
      if(upper_(c.Status)!=='CANCELLED'&&upper_(c.Status)!=='DECLINED'){
        if(!a[c.WorkID])a[c.WorkID]={a:0,c:0,t:0};
        a[c.WorkID].a++;
    ...
    regs.forEach(function(r) {
      if (r.RegID) {
        var cleanId = clean_(r.RegID);
        regMap[r.RegID] = r;
        regMap[cleanId] = r;
        regMap[cleanId.toUpperCase()] = r;
      }
    });

    rows = rows.map(function(w){
      const ac=asc[w.WorkID]||{a:0,c:0,t:0};
      const authors = (authorsByWork[w.WorkID]||[]).slice();
      authors.forEach(function(a){
        if (!clean_(a.FullName)) {
          var ap = [a.Prefix, a.FirstName, a.LastName].filter(Boolean).map(clean_);
          a.FullName = ap.join(' ') || clean_(a.AuthorFullName || a.FirstName || a.AuthorName || '');
        }
      });
      const presenter = authors.find(a=>bool_(a.IsPresenter)) || authors[0] || {};
      const regId = clean_(w.RegID || presenter.RegID || '');
      const reg = regMap[regId] || regMap[regId.toUpperCase()] || {};
      const regName = clean_(reg.FullName || [reg.Prefix, reg.FirstName, reg.LastName].filter(Boolean).map(clean_).join(' ') || reg.FirstName || '');
      const presenterName = clean_(presenter.FullName || presenter.AuthorFullName || '') || regName || clean_(w.PresenterName || '');
      const presenterEmail = clean_(presenter.Email || reg.Email || w.PresenterEmail || '');
  ...
  return runSafely_('adminListWorks', function() {
    ...
    if (!baseRows || !Array.isArray(baseRows)) {
      ...
      regs.forEach(function(r) {
        ...
        if (r.Email) {
          regEmailMap[normalizeEmail_(r.Email)] = r;
        }
      });

      baseRows = works.map(function(w) {
        var ac = asc[w.WorkID] || { a: 0, c: 0, t: 0 };
        var existingAuthors = (authorsByWork[w.WorkID] || []).slice();
        existingAuthors.forEach(function(a) {
          if (!clean_(a.FullName)) {
            var aParts = [a.Prefix, a.FirstName, a.LastName].filter(Boolean).map(clean_);
            a.FullName = aParts.join(' ') || clean_(a.AuthorFullName || a.FirstName || a.AuthorName || '');
          }
        });

        var presenter = existingAuthors.find(function(a) { return bool_(a.IsPresenter); }) || existingAuthors[0] || {};
        var presenterName = clean_(presenter.FullName || presenter.AuthorFullName || presenter.Name || '');
        if (!presenterName) {
          var pParts = [presenter.Prefix, presenter.FirstName, presenter.LastName].filter(Boolean).map(clean_);
          presenterName = pParts.join(' ');
        }

        var cat = categories[w.CategoryID] || {};
        var ptReq = ptMap[w.PresentationTypeRequested] || {};
        var ptFin = ptMap[w.PresentationTypeFinal] || {};
        var regId = clean_(w.RegID || presenter.RegID || '');
        var reg = regMap[regId] || regMap[regId.toUpperCase()] || {};

        if (!reg.RegID && regId) {
          var numMatch = regId.match(/\d+/g);
          if (numMatch && numMatch.length > 0) {
            var lastNum = numMatch[numMatch.length - 1];
            if (lastNum && lastNum.length >= 3) {
              reg = regs.find(function(r) { return String(r.RegID || '').indexOf(lastNum) >= 0; }) || {};
            }
          }
        }
        if (!reg.RegID && w.PresenterEmail) {
          reg = regEmailMap[normalizeEmail_(w.PresenterEmail)] || {};
        }

        var regFullName = clean_(reg.FullName || '');
        if (!regFullName) {
          var rParts = [reg.Prefix, reg.FirstName, reg.LastName].filter(Boolean).map(clean_);
          regFullName = rParts.join(' ');
          if (!regFullName) regFullName = clean_(reg.FirstName || '');
        }
        var regEmail = normalizeEmail_(reg.Email || '');
        var regPhone = normalizePhone_(reg.Phone || '');
        var regOrg = clean_(reg.Institution || reg.OrganizationUnit || reg.OrganizationGroup || '');

        if (!presenterName && regFullName) {
          presenterName = regFullName;
        }
        if (!presenterName) {
          presenterName = clean_(w.PresenterName || w.AuthorFullName || w.FirstAuthorName || w.AuthorName || '');
        }

        var presenterEmail = normalizeEmail_(presenter.Email || w.PresenterEmail || regEmail || '');
        var presenterPhone = normalizePhone_(presenter.Phone || w.PresenterPhone || regPhone || '');
        var presenterOrg = clean_(presenter.Organization || w.Organization || regOrg || '');

        var finalAuthors = existingAuthors;
        if (finalAuthors.length === 0 && (presenterName || regFullName)) {
          var fallbackAuthor = {
            AuthorID: 'AUTH-AUTO-' + (w.WorkID || regId || nextId_('AUTH')),
            ConferenceID: cid,
            WorkID: w.WorkID,
            AuthorOrder: 1,
            Prefix: clean_(reg.Prefix || presenter.Prefix || ''),
            FirstName: clean_(reg.FirstName || presenter.FirstName || presenterName),
            LastName: clean_(reg.LastName || presenter.LastName || ''),
            FullName: presenterName || regFullName,
            Position: clean_(reg.Position || presenter.Position || ''),
            Organization: presenterOrg,
            Email: presenterEmail,
            Phone: presenterPhone,
            IsPresenter: true,
            IsCorrespondingAuthor: true
          };
          finalAuthors = [fallbackAuthor];

          // Auto-heal WorkAuthors sheet in Google Sheets if missing
          if (w.WorkID) {
            try {
              appendRecord_('WorkAuthors', fallbackAuthor);
            } catch(healErr) {}
          }
        }
    ...
    if (filters.q) {
      ...
      rows = rows.filter(function(x) {
        return [x.WorkCode, x.RegID, x.TitleTH, x.TitleEN, x.PresenterName, x.RegistrationFullName, x.CategoryName, x.PresentationType, x.PresentationTypeName].join(' ').toLowerCase().indexOf(q) >= 0;
      });
    }
    if (filters.status) {
...
function getAdminSettings(token, conferenceId) {
  return runSafely_('getAdminSettings', function() {
    requireSession_(token, ['SUPERADMIN', 'CONFERENCE_ADMIN'], conferenceId);
    var cid = conferenceId || APP.DEFAULT_CONFERENCE_ID;
    var cacheKey = 'ADM_SETT_' + cid;
    try {
      var cached = CacheService.getScriptCache().get(cacheKey);
      if (cached) return JSON.parse(cached);
    } catch(e) {}

    var conf = findOne_('Conferences', { ConferenceID: cid });
    if (!conf) throw new Error('ไม่พบข้อมูลงานประชุม');
    var result = {
      conference: serialize_(conf),
      settings: serialize_(findMany_('Settings', { ConferenceID: cid })),
      optionConfig: getRegistrationOptionMap_(cid),
      registrationTypes: serialize_(findMany_('RegistrationTypes', { ConferenceID: cid })),
      reviewRounds: serialize_(findMany_('ReviewRounds', { ConferenceID: cid })),
      categories: serialize_(findMany_('WorkCategories', { ConferenceID: cid })),
      presentationTypes: serialize_(findMany_('PresentationTypes', { ConferenceID: cid }))
    };
    try {
...
function adminGetWorkScoreSummary(token, conferenceId, workId) {
  return runSafely_('adminGetWorkScoreSummary', function() {
    ...
    var authors = (findMany_('WorkAuthors', { ConferenceID: conferenceId, WorkID: workId }) || []).slice();
    authors.forEach(function(a){
      if (!clean_(a.FullName)) {
        var ap = [a.Prefix, a.FirstName, a.LastName].filter(Boolean).map(clean_);
        a.FullName = ap.join(' ') || clean_(a.AuthorFullName || a.FirstName || a.AuthorName || '');
    ...
    if (authors.length === 0) {
      var regId = clean_(work.RegID || '');
      var reg = (regId && (findOne_('Registrations', { ConferenceID: conferenceId, RegID: regId }) || findOne_('Registrations', { RegID: regId }))) || null;
      if (reg) {
        var rName = clean_(reg.FullName || [reg.Prefix, reg.FirstName, reg.LastName].filter(Boolean).map(clean_).join(' ') || reg.FirstName || '');
        if (rName) {
          authors = [{
            AuthorID: 'AUTH-' + (work.WorkID || reg.RegID),
            WorkID: work.WorkID,
            FullName: rName,
            Prefix: reg.Prefix || '',
            FirstName: reg.FirstName || rName,
            LastName: reg.LastName || '',
            Email: reg.Email || '',
            Phone: reg.Phone || '',
            Organization: reg.Institution || reg.OrganizationUnit || reg.OrganizationGroup || '',
            IsPresenter: true
          }];
        }
      }
    ...
    var allScores = findMany_('ReviewScores', { ConferenceID: conferenceId, WorkID: workId });
    var allCriteria = findMany_('ScoringCriteria', { ConferenceID: conferenceId }).filter(function(c){ return bool_(c.Active); });
    var criteria = filterCriteriaForWork_(allCriteria, work);
    var validCritMap = {};
    criteria.forEach(function(c){ validCritMap[c.CriteriaID] = true; });
...
const API_WRITE_ACTIONS = Object.freeze({
  ...
  adminSaveScoringCriterion:1, adminSaveReviewRound:1, adminSaveRegistrationType:1, adminSetupCategoryScoringCriteria:1,
